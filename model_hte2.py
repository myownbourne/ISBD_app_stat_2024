...
cols_nonvio = np.loadtxt('cols_nonvio2.txt', dtype=str)
...
dml_data = DoubleMLData(data_frame2, y_col='nonViolPerPop',d_cols='PersPerRentOccHous')
...
import patsy
design_matrix = patsy.dmatrix("bs(x, df=4, degree=2)", {"x": data_frame2["RentMedian"]})
...
new_data = {"x": np.linspace(-0.5, 3, 100)}
...
plt.legend()
plt.title('CATE (treatment variable - PersPerRentOccHous)')
plt.xlabel('RentMedian')
